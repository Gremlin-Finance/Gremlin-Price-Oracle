...
import "./interfaces/IGremlinPriceOracle.sol";

contract GremlinPriceOracle is IGremlinPriceOracle {
    ...
    function toUint224(uint256 input) internal pure returns (uint224) {
        require(input <= uint224(-1), "GremlinPriceOracle: UINT224_OVERFLOW");
    ...
    function initialize(address uniswapV2Pair) external {
        ...
        require(
            ...
            "GremlinPriceOracle: ALREADY_INITIALIZED"
        );
        ...
        returns (uint224 price, uint32 T)
    {
        ...
        require(pair.initialized, "GremlinPriceOracle: NOT_INITIALIZED");
        Pair storage pairStorage = getPair[uniswapV2Pair];
        ...
        require(T >= MIN_T, "GremlinPriceOracle: NOT_READY"); //reverts only if the pair has just been initialized
